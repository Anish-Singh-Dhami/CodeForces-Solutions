...
void sol () {
  ...
  // observ chess board like pattern, with finite size.
  // if both player lies in same type of cell ==> possible to meet somewhere
  for(int i = 1; i <= k; i++) {
    int xx, yy;
    cin >> xx >> yy;
    if((x + y) % 2 == (xx + yy) % 2) {  // if vika & her friend in the same type of cell
      find = true;
    }
  }
